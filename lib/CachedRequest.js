...
class CachedRequestInternal
{
    ...
    async getAllCacheFiles(url)
    {
        return await readdir.async(this.cachePath,
        {
            ...
            {
                return stats.isFile() && stats.path.endsWith(urlModule.domainToUnicode(new URL(url).hostname)+'.json');
            }
        });
    }
    ...
    async findCacheFile(url)
    {
        ...
        const files = await this.getAllCacheFiles(url);
    ...
    async saveCacheFile(url, body, duration, responseType, filename)
    {
        ...
        if (!filename || filename === '')
            filename = (this.getNanoSecTime())+'_'+urlModule.domainToUnicode(new URL(url).hostname)+'.json';
