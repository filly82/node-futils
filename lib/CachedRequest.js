...
class CachedRequest
{
    constructor(cachePath, debugOutput)
    {
        ...
        this.db = new loki(path.join(cachePath, 'CachedRequest.json'), {autosave: true});
    ...
    async saveCacheFile(url, body, duration, responseType, doUpdate)
    {
        ...
        if (doUpdate)
        {
            let oldDocument = this.dbCollection.findOne({url: url});
            if (oldDocument!=null)
            {
                oldDocument['body'] = theBodyStringBase64;
                oldDocument['duration'] = durationDate;
                this.dbCollection.update(oldDocument);
            }
            else
                this.dbCollection.insert({'url': url, 'body': theBodyStringBase64, 'duration': durationDate});
        }
        else
            this.dbCollection.insert({'url': url, 'body': theBodyStringBase64, 'duration': durationDate});
