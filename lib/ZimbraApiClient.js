...
class ZimbraApiClient
{
   ...
   {
      return this.getFData(fullPath).then(response =>
      {
         try 
         {
            return JSON.parse(cheerio.load(response).text());
         }
         catch (err)
         {
            throw new Error(err);
   ...
   async getFData(fullPath)
   {
      return this.getDataString(fullPath).then(response => 
      {
         let theResponse;
   ...
   async getDataString(fullPath)
   {
      return this.internal.getDocumentContent(fullPath).then(response => 
      {
         return Buffer.from(response, 'binary').toString('utf8');
      });
   }

   getDataBuffer(fullPath)
   {
