...
class ZimbraApiClient
{
   ...
   async getMessageFolders(myPaths)
   {
      let folders = [];
      for (const myPath of myPaths)
      {
         try
         {
            folders.push(await this.internal.getItem(myPath, MessageFolder.view));
         }
   ...
   async getMessages(messageFolder)
   {
      try
      {
         return await this.internal.search(messageFolder.id, MessageFolder.view);
      }
      catch (err)
      {
         return [];
...
class Internal
{
   ...
   search(pathId, types)
   {
      ...
      return new Promise(function (resolve, reject)
      {
         var reqObj = { "SearchRequest": { "@": { "xmlns": "urn:zimbraMail", "types": types, "limit": 1000 }, "query": "inid:" + pathId } };
